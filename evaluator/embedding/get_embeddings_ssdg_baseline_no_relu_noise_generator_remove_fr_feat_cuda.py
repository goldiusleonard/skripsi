...
from fas_simple_distill.model.resnet_ssdg.ssdg_resnet_baseline_no_relu import (
    DG_model,
    NoiseModel,
)
...
class LivenessModel:
    def __init__(self, ckpt_path, device: str = "cuda", t=0.00001):
        ...
        self.noise_model = NoiseModel(embedding_size=512)

        model_state_dict = model_ckpt["model"]
        noise_model_state_dict = model_ckpt["noise_generator"]

        self.dg_model.load_state_dict(model_state_dict)
        self.dg_model.eval()
        self.dg_model.to(self.device)
        self.noise_model.load_state_dict(noise_model_state_dict)
    ...
    def remove_feat(self, fas_embs, fr_embs):
        ...
        dist_matrices = torch.zeros((batch, embedding_size, embedding_size)).cuda()
        for batch_index in range(int(fas_embs.size(0))):
            fas_emb = fas_embs[batch_index]
            fr_emb = fr_embs[batch_index]
            for i, fas_emb_element in enumerate(fas_emb):
                result = torch.abs(fas_emb_element - fr_emb)
                dist_matrices[batch_index][i] = result
    ...
    def __call__(self, image: np.ndarray):
        ...
        _, feat = self.dg_model(input_tensor)
        fr_feat = self.fr_model(resized_input_tensor)
        noise = torch.randn_like(feat, device=self.device)
        final_noise = self.noise_model(noise)
        feat += final_noise
        feat = self.remove_feat(feat, fr_feat)
        cls_out = self.dg_model.classifier(feat, self.dg_model.norm_flag)
...
def _main():
    ...
    liveness_model = LivenessModel(
        "evaluator/weights/ssdg_no_relu_noise_generator_OMItoC.pth"
    )
    embedding_folder = "ssdg_no_relu_noise_generator_OMItoC"
    ...
    # dst_path = f"./embedding_for_train/{embedding_folder}/"
