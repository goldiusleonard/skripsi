...
class LivenessModel:
    def __init__(self, ckpt_path, device: str = "cuda"):
        ...
        self.dg_model = DG_model(
            "F:/skripsi/FAS-Skripsi-4/fas_simple_distill/model/mobilevit_config/mobilevit_s.yaml"
        )
    ...
    def __call__(self, image: np.ndarray):
        try:
            ...
            return -1, -1
        cls_out, feat, _ = self.dg_model(input_tensor)
        score = cls_out.softmax(dim=1)[..., 0]
...
def _main():
    ...
    liveness_model = LivenessModel("evaluator/weights/divt_mobilevits_OMItoC.pth")
    embedding_folder = "divt_mobilevits_OMItoC"
    ...
    for img_path in tqdm(image_paths):
        ...
        attack_type = spoof_type_to_int(str(img_path).split("\\")[-2])
