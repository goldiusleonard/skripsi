...
from fas_simple_distill.data.datapipe.mitigation import (
    ...
    DictExtractSpoofTypeIterDataPipe,
...
class SSDGBinaryTuneTrainer:
    ...
    def build_model(self):
        ...
        # -------------------------------- criterions -------------------------------- #
        self.criterion_dic = object_from_dict(self.config.hparams.criterion_dic).to(
            self.device
        )
        self.criterion_dia = object_from_dict(self.config.hparams.criterion_dia).to(
            self.device
        )
    ...
    def initialize_train_loader(self):
        ...
        train_datapipe = DictExtractSpoofTypeIterDataPipe(train_datapipe, TGT_KEY)
    ...
    def train(self):
        ...
        dia_loss_meter = AverageMeter("dia_loss")
        dic_loss_meter = AverageMeter("dic_loss")
        ...
        for epoch in range(self.current_epoch, max_epoch):
            ...
            for batch_dict in self.train_loader:
                ...
                lbl_spoof = batch_dict[SPOOF_TYPE_KEY].to(
                ...
                dic_loss = self.criterion_dic(feat, target.view(-1))
                dia_loss = self.criterion_dia(cls_out, lbl_spoof.view(-1))

                train_loss = (dic_loss * self.config.hparams.lambda_dic) + (
                    dia_loss * self.config.hparams.lambda_dia
                )
                ...
                dia_loss_meter.update(dia_loss.item(), n=minibatch_size)
                dic_loss_meter.update(dic_loss.item(), n=minibatch_size)
            ...
            log_dict = {
                ...
                "dia_loss_avg": dia_loss_meter.avg,
                "dic_loss_avg": dic_loss_meter.avg,
