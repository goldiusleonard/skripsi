...
from datatools.torch_data.webdataset import (
    ...
    get_webdataset_datapipe_v1,
)
from fas_simple_distill.data.datapipe.add_resized import AddResizedImgIterDataPipe
from fas_simple_distill.data.datapipe.face_crop import (
    FaceAlignFromDetsIterDataPipe,
    FaceCropFromDetsIterDataPipe,
)
from fas_simple_distill.data.datapipe.mitigation import (
    DictBlankImageSkipperDP,
    DictToPILIterDataPipe,
)
...
class SSDGBinaryTuneTrainer:
    ...
    def build_model(self):
        ...
        self.fr_model = object_from_dict(self.config.hparams.fr_model).to(self.device)

        if self.config.hparams.get("fr_model_pretrained_path", None) is not None:
            ckpt = torch.load(self.config.hparams.fr_model_pretrained_path)
            self.fr_model.load_state_dict(ckpt["model"])

        if self.config.data_parallel:
            self.model = torch.nn.DataParallel(self.model)
            self.fr_model = torch.nn.DataParallel(self.fr_model)

        self.fr_model.requires_grad_(False)
        ...
        # -------------------------------- criterions -------------------------------- #
        self.criterion_cls = object_from_dict(self.config.hparams.criterion_cls).to(
            self.device
        )
        self.criterion_dom = object_from_dict(self.config.hparams.criterion_dom).to(
            self.device
        )
        # self.criterion_mining = object_from_dict(self.config.hparams.criterion_mining).to(self.device)
        ...
        param_groups = [
            {
                ...
                "lr": self.config.hparams.init_lr,
            },
            {
                "params": filter(
    ...
    def _postprocess_train_datapipe(self, train_transform, train_datapipe):
        ...
        train_datapipe = AddResizedImgIterDataPipe(
            train_datapipe,
            img_key=IMG_KEY,
            size=112,
        )
    ...
    def create_val_loader(self, val_transform, url):
        ...
        if align_config := self.config.dataset.val_dataset.get("align_config"):
            val_datapipe = FaceAlignFromDetsIterDataPipe(val_datapipe, **align_config)
        ...
        val_datapipe = AddResizedImgIterDataPipe(
            val_datapipe,
            img_key=IMG_KEY,
            size=112,
        )
    ...
    def get_min_dists(self, fas_embs, fr_embs):
        ...
        min_dists = torch.abs(min_dists - 1.0)
    ...
    def train(self):
        ...
        mining_loss_meter = AverageMeter("mining_loss")
        ...
        for epoch in range(self.current_epoch, max_epoch):
            ...
            for batch_dict in self.train_loader:
                ...
                img_resized = batch_dict[RESIZED_IMG_KEY].to(
                ...
                self.adjust_learning_rate(
                    ...
                    self.init_param_lr,
                    self.config.hparams.lr_epoch_1,
                    self.config.hparams.lr_epoch_2,
                )
                # with torch.autocast(device_type=self.device, enabled=self.config.amp.enabled):
                cls_out, feat = self.model(img)
                fr_feat = self.fr_model(img_resized)
                mining_loss = self.get_min_dists(feat, fr_feat).mean()

                classif_loss = self.criterion_cls(cls_out, target.view(-1))
                ...
                train_loss = (
                    ...
                    + (mining_loss * self.config.hparams.lambda_mining)
                )
                ...
                mining_loss_meter.update(mining_loss.item(), n=minibatch_size)
            ...
            log_dict = {
                ...
                "mining_loss_avg": mining_loss_meter.avg,
    ...
    def run_validation(self):
        ...
        for val_loader, val_name, _ in self.val_loaders:
            ...
            for batch_dict in val_loader:
                img: torch.Tensor = batch_dict[IMG_KEY].to(
                    self.device, non_blocking=True
                )
                target: torch.Tensor = batch_dict[TGT_KEY].to(
                    self.device, non_blocking=True
                )
            ...
            val_log_dict = {
                ...
                f"{val_name}_NPCER_at_APCER5e-3": metrics["NPCER@APCER0.5%"] * 100,
...
def _main():
    ...
    shutil.copy(args.tune_config_path, tune_config_path)
