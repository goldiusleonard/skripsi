...
class FeatureEmbedderResNetTorch(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.embedder = nn.Sequential(self.fc, nn.Identity(), nn.Dropout(drop_rate))
...
class FeatureEmbedderiResNetTorch(nn.Module):
    def __init__(
        self,
        model_type: str = "r18",
        pretrained_path: Optional[str] = None,
        ...
        super(FeatureEmbedderiResNetTorch, self).__init__()
        ...
        self.backbone.add_module("bn2", resnet_model.bn2)
        self.backbone.add_module("flatten", nn.Flatten())
        self.backbone.add_module("dropout", resnet_model.dropout)
        self.backbone.add_module("fc", resnet_model.fc)
        self.backbone.add_module("features", resnet_model.features)
...
class DG_model(nn.Module):
    def __init__(
        self,
        model: str = "resnet18",
        cosface_model: str = "r18",
        embedding_size: int = 512,
        drop_rate: float = 0.5,
        pretrained_path: Optional[str] = None,
        pretrained_path_cosface: Optional[str] = None,
        norm_flag: bool = True,
        num_subject: int = 140,
        **kwargs
    ):
        ...
        self.fas_embedder = FeatureEmbedderResNetTorch(
            model_type=model,
        ...
        self.fas_classifier = Classifier(embedding_size=embedding_size, num_classes=2)
        self.fas_confuser = nn.Linear(512, 512)

        self.fr_embedder = FeatureEmbedderiResNetTorch(
            model_type=cosface_model,
            norm_flag=norm_flag,
            pretrained_path=pretrained_path_cosface,
        )
        self.fr_classifier = Classifier(
            embedding_size=embedding_size, num_classes=num_subject
        )
        self.norm_flag = norm_flag

        self.fas_embedder.requires_grad_(False)
        # self.fas_classifier.requires_grad_(False)

        # self.fr_embedder.requires_grad_(False)
        # self.fr_embedder.eval()

    def forward(self, fas_input, fr_input):
        fas_features: torch.Tensor = self.fas_embedder(fas_input)
        fas_features = self.fas_confuser(fas_features)
        fr_features: torch.Tensor = self.fr_embedder(fr_input)
        fas_classifier_out = self.fas_classifier(fas_features, self.norm_flag)
        fr_classifier_out = self.fr_classifier(fr_features, self.norm_flag)
        fr_cnf_fas_out = self.fas_classifier(fr_features, self.norm_flag)
        fas_cnf_fr_out = self.fr_classifier(fas_features, self.norm_flag)
        ...
        # return fas_features, fr_features, fas_classifier_out, fr_cnf_fas_out

    def fas_parameters(self, recurse: bool = True):
    ...
    def fr_parameters(self, recurse: bool = True):
        r"""Returns an iterator over module parameters.
