...
class NoiseModel(nn.Module):
    def __init__(self, embedding_size=512) -> None:
        super().__init__()
        self.embedding_size = embedding_size
        self.noise_layer1 = nn.Linear(embedding_size, embedding_size)
        self.noise_layer1.weight.data.normal_(0, 0.01)
        self.noise_layer1.bias.data.fill_(0.0)
        self.noise_layer2 = nn.Linear(embedding_size, embedding_size)
        self.noise_layer2.weight.data.normal_(0, 0.01)
        self.noise_layer2.bias.data.fill_(0.0)
...
class DG_model_noise(nn.Module):
    def __init__(
        self,
        model: str = "resnet18",
        embedding_size: int = 512,
        ...
        **kwargs
        ...
        Args:
            ...
            detach_classifier (bool, optional): Whether to computation graph of classifier from features,
                which disables gradient backpropagation from the classifier to the backbone. Defaults to False.
        """
        super(DG_model_noise, self).__init__()
        self.model = DG_model(
            model=model,
            embedding_size=embedding_size,
            drop_rate=drop_rate,
            pretrained_path=pretrained_path,
            norm_flag=norm_flag,
            detach_classifier=detach_classifier,
        )
        ...
        self.noise_model = NoiseModel(embedding_size=embedding_size)
        self.detach_classifier = detach_classifier
        self.norm_flag = norm_flag
        self.device = device
    ...
    def forward(self, x):
        ...
        noise = self.noise_model(
            torch.randn_like(features, device=self.device, requires_grad=True)
        )
        noisy_features = features + noise

        if self.detach_classifier:
            feat_for_classif = noisy_features.clone().detach()
        else:
            feat_for_classif = noisy_features
        classifier_out = self.model.classifier(feat_for_classif, self.norm_flag)
