...
class FeatureEmbedderResNetTorch(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.fr_fc = nn.Linear(self.avgpool_feat * 8 * 8, embedding_size)
        self.fr_fc.weight.data.normal_(0, 0.005)
        self.fr_fc.bias.data.fill_(0.1)

        self.norm_flag = norm_flag

    def forward(self, x):
        featmap = self.backbone(x)
        fr_features = self.fr_fc(torch.flatten(featmap, 1))
        x = self.avgpool(featmap)
        x = torch.flatten(x, 1)
        x = self.embedder(x)
        if self.norm_flag:
            x = ssdg_normalize(x)
            fr_features = ssdg_normalize(fr_features)
        return x, fr_features
...
class DG_model(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.classifier = Classifier(embedding_size=embedding_size, num_classes=2)
        self.norm_flag = norm_flag

    def forward(self, x):
        fas_features, fr_features = self.embedder(x)
        fas_classifier_out = self.classifier(fas_features, self.norm_flag)

        return fas_features, fr_features, fas_classifier_out
