...
class FeatureEmbedderResNetTorch(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.fc = nn.Linear(self.avgpool_feat * 8 * 8, embedding_size)
...
class DG_model(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.fas_classifier = Classifier(embedding_size=embedding_size // 2)
        self.fr_classifier = Classifier(
            embedding_size=embedding_size // 2, num_classes=140
        )
        self.norm_flag = norm_flag
    ...
    def forward(self, x):
        ...
        fas_classifier_out = self.fas_classifier(
            features[:, : self.embedding_size // 2], self.norm_flag
        )
        fr_classifier_out = self.fr_classifier(
            features[:, self.embedding_size // 2 :], self.norm_flag
        )

        return fas_classifier_out, fr_classifier_out, features
