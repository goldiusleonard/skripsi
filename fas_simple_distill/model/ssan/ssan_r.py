...
class Discriminator(nn.Module):
    def __init__(self, max_iter):
        ...
        self.ad_net = nn.Sequential(
            self.fc1, nn.ReLU(inplace=True), nn.Dropout(0.5), self.fc2
        )
...
class SSAN_R(nn.Module):
    def __init__(self, ada_num=2, max_iter=4000):
        ...
        self.adaIN_layers = nn.ModuleList(
            [ResnetAdaINBlock(256) for i in range(ada_num)]
        )
        ...
        self.FC = nn.Sequential(nn.Linear(256, 256, bias=False), nn.ReLU(inplace=True))
    ...
    def cal_gamma_beta(self, x1):
        ...
        domain_invariant = torch.nn.functional.adaptive_avg_pool2d(x1_4, 1).reshape(
            x1_4.shape[0], -1
        )
